...
func LevelPresetForLevel(levelId int) LevelPresetRecord {
	for idx := range(LevelPresets) {
		if LevelPresets[idx].LevelId != levelId {
			continue
		}
		return LevelPresets[idx]
	}

	return LevelPresetRecord{}
}
